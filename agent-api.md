...
> definitive, never blocks, and never presented as a fault. A codec probe reports it when the GPU
> cannot open that codec's encoder at all. Additive: `status` was always an open string that
> consumers pass through. An older console shows it as an unrecognized status (advisory, listed
...
result** (`pass`, `fail` or, since the #311 addendum, `unsupported`), with its original
`observed_at`, and may say in `summary` that a later attempt was inconclusive; it reports
`unknown` only while no definitive result exists for that `id`. Either way an indeterminate probe
neither sets nor clears a block. See `schema.md` `hosts.readiness`.
...
### `image_cleanup_v1` — exact-version cache removal (RH05 #345; proposed amendment)

A current authenticated register advertises `image_cleanup_v1: true` when the agent implements the versioned inventory protocol, exact cleanup command, and durable journal. Capability is independent of inventory completeness: an upgraded agent may advertise support while reporting `image_versions_complete: false`, and cleanup remains disabled until classification succeeds. It reports `image_versions_complete: true` and `image_versions: [{image_id, version, image_ref, runtime_image_id, state, container_referenced?}]` for every known managed version, including prior versions. Each frozen adopted ref (a digest for prebuilt images or a local tag for templates) is bound to its currently verified daemon image ID; a changed binding makes inventory unknown until reconciled. `state` is exactly `present`, `absent`, or `unknown`: present means the exact ref and daemon ID match; absent requires a complete daemon ref scan and a complete all-container scan (including stopped containers) proving no container uses that version's runtime image ID; unknown never authorizes deletion. An agent that provides preview reference evidence includes Boolean `container_referenced` on every `present` entry. It compares the container's resolved daemon image ID (the full `sha256:` ID returned by Docker with `all=true`, including created, exited, paused, dead, and restarting containers) with the entry's full normalized `runtime_image_id`; a container's configured image name or tag is not sufficient. `true` means at least one container uses that exact ID and `false` means none does in the same complete all-container scan used for absence evidence. The field is omitted on `absent` and `unknown` entries. A non-Boolean value, or a value on a non-`present` entry, makes the snapshot malformed and inventory unknown. An older cleanup-capable agent may omit the field on a `present` entry; omission is unknown, never evidence of no references. The control plane makes that candidate ineligible with `unknown_inventory` and a remedy to upgrade the node agent for container-reference evidence, then refresh inventory. No container names or IDs are sent. The value is point-in-time evidence, not a reservation: cleanup still rechecks every container under the per-image operation/launch lock immediately before non-forced removal. Creating or removing a container using a managed runtime image ID counts as an external daemon change; the agent detects it by daemon events or a bounded periodic complete rescan and publishes a new inventory revision when the reference result changes. Lost event observation requires a complete rescan or revocation of completeness. A failed or incomplete container scan revokes completeness and cannot report absent. Missing completeness, missing runtime ID, or an old connection epoch makes inventory unknown. The agent persists version/ref/runtime-ID records before pull/build, retains old records when a new version succeeds, and reconciles them on startup. Unknown daemon images are not classified as managed versions. Legacy `images[]` remains one current version and never proves an older version absent. After register, the agent sends `{"type":"image_versions_state","inventory_revision":"8","image_versions_complete":true,"image_versions":[{"image_id":"steam","version":"2026.08.07","image_ref":"<frozen-adopted-ref>","runtime_image_id":"<daemon-image-id>","state":"present","container_referenced":false}]}` as a complete replacement snapshot (`image_versions_complete`, `image_versions`, `inventory_revision` canonical decimal string) whenever pull/build/cleanup, startup reconciliation, or external daemon change alters known managed inventory. Revisions increase within one authenticated connection; the control plane ignores older revisions/epochs. Completeness is revoked immediately on a failed daemon scan and re-advertised only after a complete scan; a stale last-good snapshot is never current authority. The control plane sends `{"type":"image_inventory_reconcile","id":"<command-id>","identities":[{"image_id":"steam","version":"2026.08.07","image_ref":"<frozen-adopted-ref>"}]}` on capable registration and on history/adoption changes, including every frozen current/previous identity it knows. The agent replies with ack and a fresh `image_versions_state`; the command never authorizes removal. On upgrade from a single-current record, `image_versions_complete` starts false. It becomes true only after a full daemon ref enumeration is reconciled against the durable journal and the control plane's frozen adopted/current/previous identities; every ref in the managed namespaces must be classified or explicitly resolved by operator repair. An empty old journal alone is never completeness proof.

The control plane increments the fence generation when it creates an attempt and sends that **post-increment** generation in every command. Before terminal acknowledgement, the agent treats a duplicate `(attempt_id, generation, exact identity)` as the same journaled operation and starts no new runtime action: a live entry re-reports `removing`, and a terminal refusal repeats the original `ack{ok:false,error}` and terminal `failed` report. After terminal acknowledgement, the retired tombstone makes every later same-ID command refuse with `retired_attempt` and no side effects.

The **new** downstream command is `image_cleanup`:
```json
{"type":"image_cleanup","id":"<command-id>","attempt_id":"<uuid>","image_id":"steam","version":"2026.08.07","image_ref":"<frozen-adopted-ref>","runtime_image_id":"<daemon-image-id>","expected_generation":"7"}
```
Every identity field is required. For a valid, not-yet-retired attempt ID, every **first** definite refusal is durably recorded as terminal `failed` with its original safe refusal code under the agent-wide journal mutex **before** `ack{ok:false,error}` is sent. An already-retired duplicate returns `retired_attempt` without changing the tombstone or the prior terminal outcome. This includes a refusal discovered after the initial journal append or under the per-image lock: it atomically changes `removing` to `failed` before the ack, re-reports terminal failure, and never retries the runtime checks for that ID. If journal persistence fails, the agent sends no definite refusal ack and the control plane keeps the attempt unknown/protective. A refusal guarantees no runtime deletion begins from **that command receipt**; it does not rewrite an earlier receipt's durable result; a first-refusal error is one of `identity_mismatch`, `inventory_unknown`, `reference_in_use`, `operation_busy`, or `unsupported`. `retired_attempt` is only a duplicate reply and never states whether an earlier receipt deleted anything. The agent refuses a mismatched persisted record, changed daemon image ID, incomplete inventory, or pending local launch/image/template work. It journals the attempt before accepting and serializes on the per-image operation/launch lock. Under that lock it checks all containers, including stopped ones, by runtime image ID, and rechecks the exact ref and daemon ID immediately before non-forced `rmi` (`force:false`, `noprune:true`). Launch holds the same lock from final reference check until its container exists or launch fails. No user or template home is deleted.

`ack{ok:true}` accepts or identifies an identical durable in-flight attempt; it never proves removal. The terminal report shape is `{"type":"image_cleanup_state","attempt_id":"<uuid>","image_id":"steam","version":"2026.08.07","image_ref":"<frozen-adopted-ref>","runtime_image_id":"<daemon-image-id>","generation":"7","state":"removed|failed|unknown","reason":null}`. The terminal `image_cleanup_state` echoes attempt ID, exact image/version/ref/runtime ID and generation. It reports `removed` only after a fresh daemon lookup confirms that exact ref absent and no container depends on the target ID; `failed` means either a durable pre-delete refusal (its reason is one of the first-refusal codes above, excludes `retired_attempt`, and proves no delete began from that attempt) or that the ref remains after attempted removal; `unknown` means outcome cannot be established. An absent exact ref is idempotent only after a complete daemon check. On restart the journal is reconciled before new cleanup/launch work. The control plane can send `{"type":"image_cleanup_journal_request","id":"<command-id>","attempt_ids":["<uuid>"]}` to a capable current connection. One agent-wide durable journal mutex covers each cleanup handler's retired-ID check and journal append **before** it enters the per-image operation/launch lock, and each journal request's requested-ID absence check, tombstone append, and snapshot across all connection epochs and startup replay. For each requested ID absent from all active or terminal journal entries, the agent writes a durable retired-ID tombstone before answering. A delayed handler from any socket must acquire this same mutex, see the tombstone, and refuse with `ack{ok:false,error:"retired_attempt"}` before daemon side effects. If a handler journaled first, the snapshot contains its entry; if retirement wins, the handler can never start. Runtime removal stays under the separate per-image lock. Retired-ID tombstones survive process restart and are retained indefinitely; there is no RH05 garbage-collection command or age-based eviction. If the durable journal cannot accept another tombstone, the agent refuses the journal request with `ack{ok:false,error:"operation_busy"}`, reports no authoritative omission, and the control plane keeps the fence protective until the operator repairs journal storage. The journal request ack is sent only after every requested absent ID is durably retired. The agent acks then sends `{"type":"image_cleanup_journal","request_id":"<command-id>","retired_attempt_ids":["<requested-retired-uuid>"],"attempts":[{"attempt_id":"<uuid>","image_id":"steam","version":"2026.08.07","image_ref":"<frozen-adopted-ref>","runtime_image_id":"<daemon-image-id>","generation":"7","state":"removing"}]}`. This is a complete snapshot of all durable unacknowledged journal attempts across **every** connection epoch, plus the exact requested IDs proven retired under the same mutex; states are `removing`, `removed`, `failed`, or `unknown`. A live `removing` entry is not terminal even if a current daemon scan still sees the ref. The agent re-reports terminal `image_cleanup_state` on reconnect until the control plane durably records it and sends `{"type":"image_cleanup_state_ack","id":"<command-id>","attempt_id":"<uuid>","generation":"7"}`; the agent **must** durably retire that attempt ID under the agent-wide journal mutex before acknowledging the terminal ack. It may compact the entry body into a tombstone or retain a flagged retired entry; either form appears in `retired_attempt_ids` on journal request and never authorizes another deletion. A late duplicate then gets `ack{ok:false,error:"retired_attempt"}` before daemon effects, even if the exact ref was re-ensured. Lost acknowledgement is harmless: a repeated terminal report is idempotent. The control plane CASes terminal reports by attempt ID and generation. A changed generation cannot relabel an old report.

Legacy `image_remove` stays unchanged and is only a best-effort current-version operation. RH05 cleanup never sends it. Older agents omit `image_cleanup_v1`, so the unknown new command cannot delete their current image. A new requirement arriving during physical removal remains pending behind the fence; stale ready evidence is suppressed and a current ensure is required before launch.
