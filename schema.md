...
| `state` | `TEXT` NOT NULL | `CHECK (state IN ('pending','running','succeeded','failed','cancelled'))`. `pending` = accepted, nothing sent yet; `running` = a target is being applied; `succeeded` = **every** target succeeded; `failed` = a target failed and **the run stopped there**; `cancelled` = an admin cancelled and the run stopped before its next target. |
...
| `kind` | `TEXT` NOT NULL | `CHECK (kind IN ('apply','revert'))`. **A revert is an apply with an older digest set** — same wire message, same states, same reasons — so it is a `kind` on this row rather than a second table or a second `agent-api.md` message. The column exists so history can say which button was pressed, and for nothing else. |
...
- **No rollback pointer beyond `previous_digests`.** A revert is an ordinary attempt with `kind='revert'`; there is no `reverted_by` column, because a target's state is whatever its **latest** attempt says and a chain of pointers would be a second, disagreeable answer to that question.
