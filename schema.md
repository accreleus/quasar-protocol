...
**Hidden secrets.** A declared secret may be marked **hidden**, which keeps it off
`GET /v1/admin/secrets` and makes `PUT`/`DELETE /v1/admin/secrets/{name}` answer `404` for it —
identically to a name the build does not declare. It is a route-surface decision, not an
access-control one: both routes are admin-gated either way. It exists for a secret whose value
is only meaningful **alongside something else**, so accepting it through the generic key/value
route would install a half-configured state the dedicated endpoint exists to prevent.
`tls.private_key` (below) is the case and currently the only one.

## `instance_tls_certificate` (first-run wizard v2 §S6d, 2026-08-09)
> *Additive amendment (migration 0064). A new **singleton** table; it changes no existing
> table. Holds an operator-uploaded TLS certificate installed through
> `POST /v1/admin/tls/certificate`.*

**The PUBLIC half only.** The private key is sealed into `instance_secrets` under
`tls.private_key` (hidden, no env fallback) — **not** stored here and **not** written as a
plaintext pem beside the self-signed pair. That split is the point: a dump of this table yields
a certificate chain that was already transmitted in every TLS handshake and nothing else, while
its key is ciphertext in a different table under a key that lives only in the environment.

A **singleton**, like `instance_settings`: one listener, one served certificate. Replacing the
certificate is an upsert, so there is **no history** — deliberately. A superseded certificate is
not evidence of anything an operator needs, and keeping old chains only widens what a dump
discloses.

| column | type | notes |
|---|---|---|
| `id` | `BOOLEAN` PK DEFAULT `true` | `CHECK (id)` — the singleton idiom. |
| `certificate_pem` | `TEXT` NOT NULL | The chain as uploaded, **leaf first**. Public by construction. |
| `fingerprint_sha256` | `TEXT` NOT NULL | Derived from the leaf. Stored anyway (see below). |
| `not_before` | `TIMESTAMPTZ` NOT NULL | Derived from the leaf. |
| `not_after` | `TIMESTAMPTZ` NOT NULL | Derived from the leaf. Drives the days-to-expiry figure that catches the 90-day Let's Encrypt trap. |
| `subject_alt_names` | `TEXT[]` NOT NULL DEFAULT `'{}'` | Derived: DNS names then IP addresses. |
| `uploaded_by` | `UUID` NULL → `users(id)` ON DELETE SET NULL | `SET NULL` so deleting that admin does not delete the certificate and take HTTPS down with them — the same posture as `instance_secrets.updated_by`. |
| `uploaded_at` | `TIMESTAMPTZ` NOT NULL DEFAULT `now()` | |

The four derived columns are computed from `certificate_pem` and stored anyway because
`GET /v1/admin/access-check` reports expiry and SAN coverage on a **request path**, and
re-parsing the chain per request is work with no upside. They are rewritten from the pem on
every upload, so they cannot drift.

**The down migration drops this table but deliberately does NOT delete the sealed key.** Rolling
the schema back must not destroy a credential the operator uploaded, and a key with no
certificate row is inert — the loader only ever loads the pair together.
...
control-plane/migrations/
  ...
  0064_allowed_origins_and_tls_certificate.up.sql -- (first-run wizard v2 §S6d + §S6e, purely additive) ALTER instance_settings ADD allowed_origins TEXT[] NOT NULL DEFAULT '{}'; CREATE instance_tls_certificate (id BOOLEAN PK CHECK(id), certificate_pem TEXT, fingerprint_sha256 TEXT, not_before/not_after TIMESTAMPTZ, subject_alt_names TEXT[], uploaded_by -> users ON DELETE SET NULL, uploaded_at). **§S6e (origins):** QUASAR_ALLOWED_ORIGINS was env-only, gated exactly /v1/signal, and its 403 is STRUCTURALLY INVISIBLE TO BROWSER JS — the case it exists for (a reverse proxy that rewrites Host, so the same-origin exemption misses) is therefore an outage an operator cannot diagnose from the client. Moving it to a column makes it admin-settable and reportable by GET /v1/admin/access-check. THE ENV VAR REMAINS AN OVERRIDE, NOT A DEFAULT: when it is SET — including to the empty string, which is a deliberate "explicitly nothing" — it wins outright and the column is not consulted, which is exactly what makes the upgrade a no-op for every existing deployment. An empty array is NOT deny-all (same-origin and no-Origin requests still pass), so a fresh instance keeps working with nothing configured; '*' is refused by the PATCH handler because a wildcard is indistinguishable from having no allow-list. text[] rather than a comma-joined column because the value is a set and a joined string would invite splitting on a character a (rejected, but still submittable) entry could contain. **§S6d (certificate):** the PUBLIC half only — the private key is sealed into instance_secrets under the hidden name tls.private_key, so a dump of this table yields a chain already transmitted in every handshake and nothing else. A SINGLETON because there is one listener and one served certificate; replacing it is an upsert, so there is NO HISTORY, deliberately (a superseded certificate is not evidence anybody needs, and retaining old chains only widens what a dump discloses). The four derived columns are computed from certificate_pem and stored anyway because access-check reports expiry and SAN coverage ON A REQUEST PATH; they are rewritten from the pem on every upload so they cannot drift. uploaded_by is SET NULL, not CASCADE: deleting that admin must not delete the certificate and take HTTPS down with them. NUMBERING: 0063 was taken by a migration that was subsequently withdrawn, so this pair lands at 0064 — a number already applied to a live database must never be reused, since golang-migrate keys on the version alone. The down migration drops the table and then the column, and DELIBERATELY DOES NOT DELETE THE SEALED KEY: rolling the schema back must not destroy a credential the operator uploaded, and a key with no certificate row is inert because the loader only ever loads the pair together.
```
