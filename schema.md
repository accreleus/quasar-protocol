...
| `channel` | `TEXT` NOT NULL | `CHECK (channel IN ('stable','edge'))`. Which channel this row was discovered on. The table holds **both** at once; the instance's `instance_settings.release_channel` selects which are shown, so switching channels needs no re-detection of the other one. |
