...
| `release_channel` | `TEXT` NOT NULL DEFAULT `'stable'` | *(platform-release amendment 1, migration 0074, additive; `beta` added by amendment 3, #121, migration 0079)* `CHECK (release_channel IN ('stable','edge','beta'))`. Which platform releases the admin console is shown (`CONTEXT.md` "Channel"): `stable` = tagged, noted releases; `beta` = those **and the prereleases among them**; `edge` = whatever was last published from `release_edge_branch`, with no notes. An instance follows **one** channel at a time. **`beta` stores no rows of its own** — it selects the `stable` rows in `platform_releases` and keeps the prereleases among them, which is why that table's `channel` CHECK stays `IN ('stable','edge')` (below). Admin-settable via `PATCH /v1/admin/settings`, read per request rather than at boot, so a switch takes effect with no restart. **Default `stable`** — an instance that has never been configured must not be shown branch builds. Switching channels never deletes a `platform_releases` row; it changes which rows the release view selects, and the never-below-the-control-plane rule (ADR 0002) is what makes switching back from `edge` safe. |
| `release_edge_branch` | `TEXT` NOT NULL DEFAULT `'develop'` | *(platform-release amendment 1, migration 0074, additive)* the branch the **edge** channel follows. Validated by the PATCH handler (not by a CHECK, which cannot express it): non-empty, ≤ 255 characters, and a valid git ref name component — no whitespace, no `..`, no leading `-`, no control characters; `400 validation_failed` otherwise. Stored and validated whatever the channel is, and **never cleared by a channel switch**, so an operator who visits `stable` and comes back keeps their branch. Selects nothing while `release_channel` is `'stable'` or `'beta'`, both of which read GitHub Releases. |
| `release_webhook_enabled` | `BOOLEAN` NOT NULL DEFAULT `false` | *(release notifications, amendment 4, migration 0080, additive)* whether a detected platform release is POSTed to `release_webhook_url`. **Default `false`** — an instance that has never been configured announces nothing, so the migration is a behavioural no-op on upgrade. Admin-settable via `PATCH /v1/admin/settings`, which refuses `true` with no URL: a switch that silently does nothing is worse than a refusal. Read per pass, not at boot, so switching it off stops delivery with no restart. |
| `platform_auto_apply` | `BOOLEAN` NOT NULL DEFAULT `false` | *(unattended apply, amendment 8, migration 0081, additive)* whether a detected platform release is applied **without a click**. **Default `false`**, so the migration is a behavioural no-op on upgrade. Admin-settable via `PATCH /v1/admin/settings`; unlike `release_webhook_enabled` it refuses nothing, because it depends on nothing else being configured first — with nothing to apply it applies nothing and records why in the detection run's summary. **There is deliberately no window column beside it**: an unattended pass runs from a successful `platform.release_detect` job, so that job's schedule IS the window, and a second time model here could disagree with it. Read per pass, not at boot. |
| `release_webhook_url` | `TEXT` NOT NULL DEFAULT `''` | *(release notifications, amendment 4, migration 0080, additive)* where one release notification is POSTed; `''` = none configured. **No CHECK**: a CHECK cannot express "https, no userinfo, a host we are willing to dial", and half of that rule in SQL would read as the whole rule. The PATCH handler validates it (absolute `https`, no userinfo, ≤ 2048 characters) and the delivery path re-checks it, so a row edited by hand still cannot become an unguarded request. **Treated as a credential** — a Slack or Discord webhook URL authenticates by being known — so it never reaches a log line, an audit record or a delivery error. **The signing secret is NOT here**: it is an `instance_secrets` row, `platform.release_webhook.secret`, encrypted under a key that is not in the database. |
...
| `codecs` | `JSONB` NULL | *(multi-codec, migration 0031, additive; **reworded by amendment 12, #296**)* last-reported **host codec set** (`agent-api.md` `capacity.codecs`): since amendment 12 the union of the per-GPU codec sets (`gpus.codecs`) over the host's GPUs with `encode_slots_total > 0`, as the agent reports it — the control plane stores the reported value and does not recompute it (it was "the **wire** codec set the host's active encoder path can produce"). It is also what a GPU with `gpus.codecs` NULL inherits. A JSON array subset of `["h264","h265","av1"]`. NULL ⇒ the control plane assumes `['h264']` (an old agent that never reports the field is H.264-only). Keep-if-absent on report (a later `capacity` that omits the key does not clobber the stored value). |
| `readiness` | `JSONB` NULL | *(first-run-experience §S1, migration 0062, additive; amended by amendment 11, #260)* the agent's last-reported host-readiness report (`agent-api.md` `capacity.readiness`): a JSON array of readiness checks `{id, status, summary, remediation}` plus the optional `observed_at` / `source` / `blocks`. **NULL** = no amendment-aware agent has reported yet, distinct from **`[]`** = reported, nothing to say. Keep-if-absent on report, exactly like `storage`/`effective_settings`. The check set is agent-owned — this column stores whatever the agent sends, **verbatim and never re-encoded**, so a field a newer agent adds survives to the console. ~~Never load-bearing.~~ **Since amendment 11 it is the input to the readiness gate** — but the scheduler never parses it: admission reads only the derived `readiness_block_*` columns below and `gpus.readiness_blocked`, which the control plane recomputes from this value whenever it or the host's overrides change. |
| `readiness_reported_at` | `TIMESTAMPTZ` NULL | *(first-run-experience §S1, migration 0062, additive)* when the agent **last reported** `readiness` — stamped on every real report, not only on a change (the shipped behaviour since 0062; the earlier "last changed" wording was wrong). It lets the UI show a report as stale, and since amendment 11 it is the readiness gate's freshness signal: older than the staleness window ⇒ the gate abstains. |
| `readiness_block_host` | `BOOLEAN` NOT NULL DEFAULT `false` | *(amendment 11, migration 0085, additive, **derived**)* true while the stored report has a check with `blocks.scope = "host"`, status `fail`, and no override. Written only by the verdict function, in the same transaction as the `readiness` write or the override change that caused it. Admission excludes the host while it is true **and** the report is fresh. **Not served** — the host body's `readiness_gate` is computed from `readiness` at read time by the same function. |
...
| `driver_identity` | `TEXT` NULL | *(#144, migration 0078)* opaque fingerprint of this GPU's driver/encode stack, as reported in `agent-api.md` `capacity.gpus[].driver_identity`. **NULL means unknown**, never "no driver". Replaced wholesale with the reported GPU set. Read at launch and stamped onto each `host_encoder_certification` row written for this GPU. |
| `codecs` | `JSONB` NULL | *(amendment 12, #296, migration 0086, additive)* the **GPU codec set** as reported in `agent-api.md` `capacity.gpus[].codecs`: a JSON array subset of `["h264","h265","av1"]`, `h264` always present on a usable GPU. **NULL ⇒ inherit `hosts.codecs`** (and a host with that NULL too is h264-only) — which is what every row existing when 0086 runs, and every row written from an older agent's report, holds, so no backfill is needed and a fleet of older agents behaves exactly as before. Written **wholesale with the GPU row** on each `capacity` upsert, like `render_node` and `driver_identity`: a report that omits it stores NULL, with no keep-if-absent rule. Every read that needs a GPU's set (the placement codec constraint and preference, the `GET /v1/me/profiles` union, `GET /v1/hosts/{id}/gpus`) resolves the GPU→host inheritance the same way. Only the launch-side reads apply the final `['h264']` fallback for a GPU whose host never reported either; the admin GPU list serves `null` in that case (`openapi.yaml` `GPUAvailability.codecs`), because "never reported" and "reported h264 only" need different operator advice. |
| `readiness_blocked` | `BOOLEAN` NOT NULL DEFAULT `false` | *(amendment 11, #260, migration 0085, additive, **derived**)* true while the host's stored readiness report has a check with `blocks.scope = "gpu"`, `blocks.gpu_index` = this GPU's `index`, status `fail`, and no override. Written only by the verdict function together with `hosts.readiness_block_*`; admission excludes the GPU while it is true **and** the host's `readiness_reported_at` is fresh. A GPU index the report names but the host does not have is ignored. Not served. |
...
## `host_readiness_overrides` (evidence-gated readiness, amendment 11, #260, migration 0085)
> *Additive amendment. A new table; it changes no existing table. One row is an admin's recorded
> decision to launch on a host despite one named failing readiness check (`control-api.md`
> "Readiness override"). Its own table rather than a `host_settings` knob: the knob catalog is a
> closed set of scalar types with no map, and an override has an author, a time and a lifecycle
> (it lapses) that a knob has not.*

| Column | Type | Notes |
|---|---|---|
| `host_id` | `UUID` NOT NULL REFERENCES `hosts(id)` ON DELETE CASCADE | |
| `check_id` | `TEXT` NOT NULL | the agent-owned readiness check id, stored as sent. `CHECK (length(check_id) BETWEEN 1 AND 128)`. Not a foreign key to anything: check ids live inside `hosts.readiness`. An id the current report no longer contains makes the row **inert**, not invalid. |
| `created_by` | `UUID` NULL REFERENCES `users(id)` ON DELETE SET NULL | the admin who set it. |
| `created_at` | `TIMESTAMPTZ` NOT NULL DEFAULT `now()` | |

`PRIMARY KEY (host_id, check_id)` — which is what makes `PUT` idempotent. **Lapse:** the row is
deleted by the control plane, in the transaction that stores a readiness report in which that
`check_id` has status `pass`. Rows are otherwise removed only by an admin's `DELETE` or the host's
deletion. **Rollback note:** the down migration drops the table and the three derived columns; the
loss is the admins' override decisions, and the effect is that nothing blocks — an older control
plane does not read readiness at all.
...
| `agent_version` | `TEXT` NULL | `hosts.agent_version` at measurement time. **Recorded, not matched** — matching it strictly would invalidate every certification on each agent update, which is a policy decision and not this table's job. |
| `driver_identity` | `TEXT` NULL | *(#144, migration 0078)* `gpus.driver_identity` at measurement time, written from the GPU row by the upsert itself so it cannot be stamped with an identity the host was not reporting. A measurement describes one silicon + driver + encode-stack combination, so the launch path applies a row only when this matches the GPU's current identity. **NULL means unknown and stays eligible** — every pre-0078 row is NULL, and a host whose agent reports no identity keeps its measurements. |
...
its down path.
Migration `0078_gpu_driver_identity`: adds the nullable `driver_identity` column here and on
`gpus`. No backfill: every existing row reads as unknown and stays applicable, and is replaced —
carrying an identity — the next time that (host, gpu, encoder, rung, bitrate) is certified. The key
is **unchanged**; identity is a property of the measurement, not a new dimension of it, so a
re-certification after a driver change replaces the row rather than forking a second one.

**What matching means at launch.** A stored row applies only when its `encoder` and
`driver_identity` both describe the encode path the session will run on. Either side unknown — the
host has not reported an encoder or an identity, or the row predates 0078 — matches everything:
dropping a cap outright would start the session at a rung the host may not sustain, which is the
worse failure. The rule lives in one function (`pickCert`, `control-plane/internal/session/stream_plan.go`),
with its SQL twin `CertForRung` held to the same answers by `TestCertForRungMatchesPickCert`.
...
| `template_publish_claim_token` | `UUID` NULL *(RH05 #344, 0093)* | Random per-claim token for a capable `template.warmup` runner. Replaced atomically on every claim; internal, absent from public job reads/logs/diagnostics. |
| `template_publish_connection_id` | `TEXT` NULL *(RH05 #344, 0093)* | The host's current database source-policy registration epoch at claim. A reconnect changes the host epoch and invalidates an older worker. Internal only. |
| `publish_permit_accepted_at` | `TIMESTAMPTZ` NULL *(RH05 #344, 0093)* | Stamped by the conditional final-permit SQL statement only when all current predicates hold. Evidence for a later successful run report, never reusable authorization or proof that publication completed. Internal only. |
...
## `platform_releases` (platform-release amendment 1, migration 0074)
> *Additive amendment (#104/#106, migration 0074). A new table; it changes no existing table.
> One row per **platform release** this instance's detector has seen — the control-plane and
> node-agent images built from one commit and published together (`CONTEXT.md` "Platform
> release"). It is a **cache of what exists**, never a record of what was done: the apply-history
> table belongs to amendment 2. Read by `control-api.md` §Platform releases.*

| column | type | notes |
|---|---|---|
| `id` | `UUID` PK | stable across detections — the handle the release view serves as `PlatformRelease.id` and amendment 2's apply will name. |
| `channel` | `TEXT` NOT NULL | `CHECK (channel IN ('stable','edge'))` — **unchanged by amendment 3.** Which source this row was discovered from. The table holds **both** at once; the instance's `instance_settings.release_channel` selects which are shown, so switching channels needs no re-detection of the other one. `beta` is not a value here: every GitHub Release, prerelease or not, is stored on `'stable'` carrying its `prerelease` flag, and `beta` is the channel that selects those rows *without* filtering the flagged ones out. A `'beta'` row would be a second copy of a `'stable'` one under a second key, and switching channels would then mean re-detecting. |
| `version` | `TEXT` NULL | stable semver **without** a leading `v` (`'0.2.0'`, `'0.2.0-rc.1'`). **NULL on `edge`** — an edge build is a commit, not a version, and a synthesized one would be rendered to an operator as if it were real. |
| `source_commit` | `TEXT` NOT NULL | full 40-character lowercase hex. The identity on both channels, which is why it is NOT NULL while `version` is nullable. |
| `built_at` | `TIMESTAMPTZ` NOT NULL | when the release was built; the **second** ordering key. |
| `schema_version` | `INTEGER` NOT NULL | the highest migration version the release's control-plane image embeds. The **first** ordering key and the ADR 0002 gate: a release whose `schema_version` is below the installed control plane's is never offered, on either channel, because the control plane cannot boot against a database ahead of it. |
| `prerelease` | `BOOLEAN` NOT NULL DEFAULT `false` | true for a prerelease tag. Stored so a release candidate is *discoverable* while the `stable` channel filters it out of the view. |
| `manifest` | `JSONB` NULL | the **release manifest** asset (`platform-release-manifest.json`) **verbatim**, exactly as fetched — `control-api.md` `ReleaseManifest`. **NULL on `edge`**, which publishes no asset. Stored whole rather than exploded into columns for the same reason `hosts.readiness` is: the asset's format is versioned by its own `format_version` and can grow without a migration, and a row that has been reshaped can no longer be shown to be the thing that was published. |
| `notes` | `TEXT` NOT NULL DEFAULT `''` | the release notes, markdown, verbatim from the GitHub Release's `body` field. **`''` on `edge`** (no notes exist) — NOT NULL with a default so every consumer renders one type. Untrusted upstream text: sanitized at render, never at write, so what is stored stays what was published. |
| `compare_url` | `TEXT` NULL | on `edge`, the GitHub compare link between the installed and this build's commits — what stands in for absent notes. NULL on `stable`. |
| `discovered_at` | `TIMESTAMPTZ` NOT NULL DEFAULT `now()` | when **this instance** first saw the release. Provenance, not ordering: two instances can legitimately disagree, and ordering is `schema_version` then `built_at`. |

**Constraints.**
- `UNIQUE (channel, source_commit)` — one row per build per channel, and the detector's idempotency
  key: re-running detection re-observes the same commits and must not accumulate duplicates. It is
  `(channel, source_commit)` rather than `source_commit` alone because the same commit can
  legitimately be published to both channels (an edge build that is later tagged).
- `UNIQUE (version) WHERE version IS NOT NULL` — a partial unique index. A semver names exactly one
  release; two rows claiming `0.2.0` means something upstream is wrong and must fail loudly rather
  than be shown twice. Partial because every `edge` row has a NULL `version` and a plain unique
  index would be satisfied by them anyway (NULL ≠ NULL) — the partial form states the intent.

**Detection semantics this table exists to support.**
- **The last known list survives a failed detection.** A failure records the job's error (and
  `last_error` on the view) and writes **nothing** here — an unreachable GitHub or registry must not
  empty an operator's Releases page.
- **Detection never deletes a row.** A row is a fact about a published release, and a release that
  scrolls off the upstream listing did not stop existing. Rows leave only by an operator-run
  cleanup, which this amendment does not add.
- **Nothing here is an apply record.** No column says what was installed, when, or by whom; the
  installed side is `hosts`' identity columns and the control plane's own build stamps. Amendment 2
  adds the per-target apply history, and it will be a separate table for exactly this reason.

## `platform_release_notifications` (release notifications, amendment 4, migration 0080)
> *Additive amendment (#123, migration 0080). A new table; it changes no existing table. One row
> per platform release an outbound notification has been **attempted** for. Read and written by
> `control-api.md` §"Release notifications".*

Detection runs weekly **and** on demand ("Check now"), so the same release is re-observed
indefinitely. This table is what makes an announced release stay announced.

| column | type | notes |
|---|---|---|
| `release_id` | `UUID` PK → `platform_releases(id)` **ON DELETE CASCADE** | the release this row is the notification record for. **The key is the release, not a timestamp watermark**: a watermark ("notified everything published before T") silently swallows a release published out of order or re-detected after a channel switch, and the identity of the thing announced is the only key that cannot drift. **CASCADE** because `platform_releases` is a cache — a notification record for a release that no longer exists would suppress nothing and explain nothing. (The opposite call from `platform_apply_runs.release_id`, which is RESTRICT, and for the opposite reason: that table records what was *done*, this one records only that a message was sent.) |
| `status` | `TEXT` NOT NULL | `CHECK (status IN ('delivered','failed'))`. **`delivered` is terminal** — the release is never announced again. `failed` is retried on the next detection pass until the attempt cap. |
| `attempts` | `INTEGER` NOT NULL DEFAULT `0` | `CHECK (attempts >= 0)`. How many detection **passes** this release has cost, **not HTTP requests** — the retries within one pass are the server's business. It is what bounds a permanently-broken URL to a finite amount of noise; the cap itself is a constant in the server, not a CHECK, because a schema that also encoded it would be a second place to change it. |
| `last_attempt_at` | `TIMESTAMPTZ` NOT NULL DEFAULT `now()` | when the last attempt ran. The ordering key for "the last delivery on this instance". |
| `last_status_code` | `INTEGER` NULL | the receiver's HTTP status, or **NULL when the request never got one** (DNS, TLS, a refused dial, the egress allowlist). Those are different failures and an operator reading this needs to tell them apart. |
| `last_error` | `TEXT` NULL | bounded operator prose, NULL on success. **Never the webhook URL** and never the signing secret: nothing that writes this row has either in scope. |
| `delivered_at` | `TIMESTAMPTZ` NULL | set once, on the delivery that succeeded, and never rewritten. |
...
## `platform_apply_runs` (platform-release apply, amendment 2, migration 0075)
> *Additive amendment (#104/#114, migration 0075). A new table; it changes no existing table.
> One row per **fleet apply** — an admin asking that one platform release be applied across the
> instance: the control plane first, then every eligible host in sequence (ADR 0002). Where
> `platform_releases` (amendment 1) is a cache of **what exists**, this is a record of **what was
> done**, which is why they are separate tables. Read and written by `control-api.md`
> §"Platform-release apply".*

| column | type | notes |
|---|---|---|
| `id` | `UUID` PK | the run handle: `GET /v1/admin/platform/apply/runs/{id}`, and `POST .../cancel`. |
| `release_id` | `UUID` NOT NULL → `platform_releases(id)` **ON DELETE RESTRICT** | the release being applied. **RESTRICT, not CASCADE or SET NULL**: detection never deletes a row, so the only thing that could remove one is an operator cleanup, and a cleanup must not be able to silently erase the record of a fleet-wide change. A run always names a real release row — unlike an *attempt*, which may name none (below). |
| `state` | `TEXT` NOT NULL | `CHECK (state IN ('pending','running','succeeded','succeeded_partial','failed','cancelled'))` *(widened by migration 0083, amendment 9)*. `pending` = accepted, nothing sent yet; `running` = a target is being applied; `succeeded` = **every** target succeeded and nothing behind the release was passed over; `succeeded_partial` = nothing failed, but at least one host in `skipped` carries a reason other than `up_to_date` — the fleet is on mixed versions (terminal; NOT a failure, so it never feeds the unattended suppression below); `failed` = a target failed and **the run stopped there**; `cancelled` = an admin cancelled and the run stopped before its next target. |
| `retry_of` | `UUID` NULL → `platform_apply_runs(id)` ON DELETE SET NULL | *(migration 0083, amendment 9, #185/#190)* the `succeeded_partial` run this one was started to finish, when an admin pressed "Retry skipped hosts". **Provenance only** — the retry is an ordinary fleet apply of the same release and decides its hosts at their turn exactly as any run does; nothing reads this column to choose a target. SET NULL so deleting the original leaves the retry's record standing. Served as `retry_of`. |
| `skipped` | `JSONB` NOT NULL DEFAULT `'[]'` | *(migration 0083, amendment 9, #185/#190)* the hosts the run passed over, `[{host_id, node_name, reason}]` in the order they were reached — the served `skipped` array, which was held in memory before this migration and lost on a crash. Persisted because `succeeded_partial` is DECIDED from it and a partial run whose explanation evaporated is a state with no reason. `CHECK (jsonb_typeof(skipped) = 'array' AND octet_length(skipped::text) <= 16384)`. |
| `force` | `BOOLEAN` NOT NULL DEFAULT `false` | the admin's decision, applied to **every host target** in this run: skip the wait for zero sessions and kill whatever is running. Recorded on the run *and* copied onto each attempt, so history stays readable after a run row is read out of context. |
| `unattended` | `BOOLEAN` NOT NULL DEFAULT `false` | *(unattended apply, amendment 8, migration 0081, additive)* the detection schedule started this run, not an admin (#122). **THIS COLUMN IS WHAT MAKES THE FAILURE POLICY EXPRESSIBLE**: an unattended run that failed must not be retried on the next pass, and "has unattended apply already failed on this release" cannot be answered from a history that does not record who started the run. `requested_by` cannot stand in — it is NULL for an unattended run *and* for one whose requesting admin has since been deleted (it is ON DELETE SET NULL). The suppression is per **release**: a newer release is still tried, and an admin's own apply of the failed one clears it, so one flaky host cannot end automatic updates for the instance. An unattended run is never `force` and is only ever created for a release whose `migrates` is false. |
| `requested_by` | `UUID` NULL → `users(id)` ON DELETE SET NULL | the admin who started it. SET NULL and not CASCADE for the reason every audit-shaped column here is: deleting an operator must not delete the record of what they did. |
| `cancel_requested` | `BOOLEAN` NOT NULL DEFAULT `false` | the cancel **flag**, not the cancel state. Set by `POST .../cancel`; the run reads it **between targets** and never mid-attempt. It is a persisted flag rather than an in-memory signal so a cancel survives a control-plane restart — which matters here more than anywhere, because a fleet run's first target *is* a control-plane restart. |
| `cancel_requested_at` | `TIMESTAMPTZ` NULL | when. |
| `current_target` | `TEXT` NULL | `CHECK (current_target IS NULL OR current_target IN ('control_plane','host'))`. Which target the run is on right now; NULL when `pending` or terminal. Denormalized from the attempts deliberately: it is what a resuming control plane reads first after a restart, before it has loaded the attempt rows. |
| `current_host_id` | `UUID` NULL → `hosts(id)` ON DELETE SET NULL | the host of `current_target='host'`; NULL otherwise. |
| `error` | `TEXT` NOT NULL DEFAULT `''` | operator prose for a run-level failure that belongs to no attempt (nothing to do, the release stopped being offered mid-run). `CHECK (octet_length(error) <= 4096)` — the ceiling `admin_activity.details` and `job_runs.summary` already use. |
| `created_at` | `TIMESTAMPTZ` NOT NULL DEFAULT `now()` | |
| `started_at` | `TIMESTAMPTZ` NULL | when the first target was reached. |
| `finished_at` | `TIMESTAMPTZ` NULL | when the run became terminal. |
| `cordoned_hosts` | `JSONB` NOT NULL DEFAULT `'[]'` | *(migration 0076)* what the run found before it cordoned: `[{host_id, was_cordoned}]`, one entry per host. **An entry is written before the cordon it describes and, once written, is never rewritten or re-derived from a later status read** — the run's own control-plane step restarts this process, and a re-read afterwards would record the run's own cordons as the operator's (#140). The array itself grows in two ways: the whole fleet is recorded at once before a control-plane step, and on a run that has no control-plane step (its control plane is already on the release) each host is recorded immediately before its own host step (#200) — that run took cordons through its host attempts while recording nothing, so its finish had nothing to lift. A host already present is left exactly as first written. Restored on every terminal path, including after the restart the run itself causes. **Persisted rather than held in memory because the run's FIRST TARGET RESTARTS THIS PROCESS** — holding it in memory loses it exactly when the restore needs it, and the fleet is then left `draining` with nothing that knows to lift it (observed live: a failed run left every host out of scheduling). **Not served**: `PlatformApplyRun` gains no field, so `control-api.md` is unchanged. |
| `cordons_restored_at` | `TIMESTAMPTZ` NULL | *(migration 0084, amendment 10, #176)* when this run's scheduling changes were **proven** undone — every host it cordoned back in scheduling, every admin cordon it found put back. NULL on a **terminal** run whose `cordoned_hosts` is non-empty is a **recovery requirement**, and the next control-plane start retries it. Needed because `finish` writes the terminal state BEFORE it restores the cordons, and `ActiveRun` selects only non-terminal runs — so a restore that failed, or a process that died in that window, left hosts `draining` with nothing that would ever look again. **Not served**: `PlatformApplyRun` gains no field, so `control-api.md` is unchanged. |
| `cordon_restore_attempted_at` | `TIMESTAMPTZ` NULL | *(migration 0084, amendment 10, #176)* when the boot sweep last tried this run's cleanup. An **ordering key, not a record**: the sweep is bounded per start (20 runs, one pass per start), so a plain newest-first selection would re-pick the same handful of permanently-failing runs forever and an older requirement behind them would never be looked at again. Never-attempted rows are claimed first, then the least recently attempted. **Not served.** |

**Constraints.**
- `CREATE UNIQUE INDEX platform_apply_runs_active_uk ON platform_apply_runs ((1)) WHERE state IN ('pending','running')` — **at most one active fleet run per instance, enforced by the database and not by code**, exactly as `job_runs_open_per_target` enforces single-flight for jobs. This *is* the `run_active` refusal (`control-api.md`): two admins pressing Apply at the same moment produce a unique violation, not two fleets moving at once. The index is on the constant expression `(1)` because the uniqueness is instance-wide — there is no column to key it on.
- `INDEX platform_apply_runs_history_idx (created_at DESC, id DESC)` — the run-list read, newest first.

## `platform_apply_attempts` (platform-release apply, amendment 2, migration 0075)
> *Additive amendment (#104/#114, migration 0075). One row per **attempt to move one target** —
> the control plane, or one host — to one digest set. Every apply produces a row: fleet or
> standalone, apply or revert, success or failure. This is the apply history
> (`GET /v1/admin/platform/attempts`) and it is the only durable record of the **previous**
> digests a target was on, which is what makes a revert possible at all.*
...
| `finished_at` | `TIMESTAMPTZ` NULL | when the attempt became terminal. |
| `updated_at` | `TIMESTAMPTZ` NOT NULL DEFAULT `now()` | last state write; `set_updated_at` trigger, as elsewhere. |

**Constraints.**
- `CHECK ((target = 'host') = (host_id IS NOT NULL))` — the control-plane target has no host and a host target always has one. Expressible as a row `CHECK` (unlike `job_runs`' host/scope agreement, which straddles two tables), so it is one.
- `CREATE UNIQUE INDEX platform_apply_attempts_open_target_uk ON platform_apply_attempts (COALESCE(host_id, '00000000-0000-0000-0000-000000000000'::uuid)) WHERE state NOT IN ('succeeded','failed','cancelled')` — **at most one open attempt per target**, which *is* the `attempt_in_flight` refusal and the wire's "single-flight per host: refuse, never queue". In the database rather than in code, so "the admin clicked twice" and "the fleet run reached a host an operator had just applied by hand" are impossible rather than merely unlikely. The zero UUID stands in for the control-plane target for the same reason `job_runs_open_per_target` uses it: a plain partial index on a nullable column would **not** make the NULL rows collide (NULL is never equal to NULL), so the control plane could otherwise have two open attempts.
- `CREATE UNIQUE INDEX platform_apply_attempts_request_uk ON platform_apply_attempts (updater_request_id) WHERE updater_request_id IS NOT NULL` — a `release_state` maps to **exactly one** attempt. Partial, because a queued attempt has no request id yet and those NULLs must not collide.
- `INDEX platform_apply_attempts_history_idx (created_at DESC, id DESC)` — the unfiltered history read.
- `INDEX platform_apply_attempts_host_idx (host_id, created_at DESC)` — the `?host_id=` history read, and the revert lookup ("this host's last succeeded attempt").

**What these two tables deliberately do not hold.**
- **No schedule and no policy.** Nothing here says *when* an instance should apply a release. Applies are operator-initiated; detection is the thing that runs on a schedule, and it rides the jobs framework (amendment 1).
- **No rollback pointer beyond `previous_digests`.** A revert is an ordinary attempt with `kind='revert'` (or `'auto_revert'`); there is no `reverted_by` column, because a target's state is whatever its **latest** attempt says and a chain of pointers would be a second, disagreeable answer to that question. (A run's `retry_of` is not a counter-example: it is provenance on the *run*, read by nothing that decides a target.)
- **No copy of the release manifest.** `requested_digests` is what was applied; the manifest stays on `platform_releases`, once.
...
result file's name, layout and field spellings, its atomic tmp+rename write, the log file beside
it — is **explicitly NOT a frozen interface and not part of any contract in this repository**. It
is internal to a single host's stack, both ends of it ship in the same platform release, and it is
expected to change without an amendment. **Nothing outside a host ever speaks it**: the control
plane learns an apply's progress only through `agent-api.md` `release_state` (or, for its own
target, by polling `platform_apply_attempts.updater_request_id` against its own host's updater
...
control-plane/migrations/
  ...
  0074_platform_release_identity.up.sql -- (platform-release amendment 1, #104/#106, PURELY ADDITIVE — four nullable columns, one new table, two defaulted settings columns; no existing column changes, no backfill, no behaviour change for any component that never reports or reads them.) ALTER hosts ADD source_commit TEXT NULL + built_at TIMESTAMPTZ NULL + install_mode TEXT NULL CHECK (install_mode IS NULL OR install_mode IN ('registry','source')) + updater_present BOOLEAN NULL — the identity an amendment-aware agent reports on `register` (`agent-api.md`), REPLACED WHOLESALE on every register with absent ⇒ NULL, which is the one place this differs from every keep-if-absent column beside it: identity describes the binary connected right now, so a downgrade to a pre-amendment agent must read as unknown rather than keep a commit nothing is running. CREATE platform_releases (id UUID PK, channel CHECK IN ('stable','edge'), version TEXT NULL, source_commit TEXT NOT NULL, built_at TIMESTAMPTZ NOT NULL, schema_version INTEGER NOT NULL, prerelease BOOLEAN NOT NULL DEFAULT false, manifest JSONB NULL, notes TEXT NOT NULL DEFAULT '', compare_url TEXT NULL, discovered_at TIMESTAMPTZ NOT NULL DEFAULT now()) + UNIQUE (channel, source_commit) + UNIQUE INDEX ON (version) WHERE version IS NOT NULL — the detector's cache of what has been published, never a record of what was applied (that table is amendment 2). ALTER instance_settings ADD release_channel TEXT NOT NULL DEFAULT 'stable' CHECK IN ('stable','edge') + release_edge_branch TEXT NOT NULL DEFAULT 'develop' — both defaulted, so the singleton row upgrades in place with no seeding step and a fresh instance follows the stable channel. schema_version IS THE ORDERING KEY EVERYWHERE, not semver and not built_at: it is the only one with a consequence, since boot's m.Up() crash-loops against a database ahead of the binary (ADR 0002). NUMBERING: 0074 is PROVISIONAL — this contract is authored ahead of the SQL, which lands with the detection/read tickets (#107/#110); 0072 and 0073 are taken by the host-enrollment pair, so 0074 is the next free number at authoring time. If it has been taken by then, the SQL lands at the next free number and this header records the correction rather than reusing a version, exactly as 0045 and 0064 did — a number already applied to a live database must never be reused, since golang-migrate keys on the version alone. The down migration drops the four hosts columns, the two instance_settings columns, and the table; the loss is the detected-release cache (re-detected on the next run) and every host's reported identity (re-reported on the next register), so nothing unrecoverable.
  0075_platform_release_apply.up.sql -- (platform-release apply, amendment 2, #104/#114, PURELY ADDITIVE — two new tables and nothing else; no existing column changes, no backfill, no behaviour change for any component that never applies a release. See agent-api.md §`release_apply` / §`release_state`, control-api.md §"Platform-release apply", and §`platform_apply_runs` / §`platform_apply_attempts` above.) CREATE platform_apply_runs (id UUID PK, release_id UUID NOT NULL -> platform_releases ON DELETE RESTRICT, state CHECK IN ('pending','running','succeeded','failed','cancelled'), force BOOLEAN NOT NULL DEFAULT false, requested_by UUID NULL -> users ON DELETE SET NULL, cancel_requested BOOLEAN NOT NULL DEFAULT false, cancel_requested_at, current_target TEXT NULL CHECK (NULL OR IN ('control_plane','host')), current_host_id UUID NULL -> hosts ON DELETE SET NULL, error TEXT NOT NULL DEFAULT '' CHECK octet_length <= 4096, created_at, started_at, finished_at) + UNIQUE INDEX platform_apply_runs_active_uk ON ((1)) WHERE state IN ('pending','running') + INDEX platform_apply_runs_history_idx (created_at DESC, id DESC). CREATE platform_apply_attempts (id UUID PK, run_id UUID NULL -> platform_apply_runs ON DELETE CASCADE, kind CHECK IN ('apply','revert'), target CHECK IN ('control_plane','host'), host_id UUID NULL -> hosts ON DELETE CASCADE, release_id UUID NULL -> platform_releases ON DELETE SET NULL, requested_digests JSONB NOT NULL, previous_digests JSONB NOT NULL DEFAULT '[]', updater_request_id UUID NULL, state CHECK IN ('queued','waiting_sessions','pending','pulling','recreating','verifying','succeeded','failed','cancelled'), reason TEXT NULL, sessions_remaining INTEGER NULL CHECK (NULL OR >= 0), force BOOLEAN NOT NULL DEFAULT false, output TEXT NOT NULL DEFAULT '' CHECK octet_length <= 8192, requested_by UUID NULL -> users ON DELETE SET NULL, created_at, started_at, finished_at, updated_at + set_updated_at trigger) + CHECK ((target = 'host') = (host_id IS NOT NULL)) + CHECK ((state = 'failed') = (reason IS NOT NULL)) + the two 4096-byte JSONB bounds + UNIQUE INDEX platform_apply_attempts_open_target_uk (COALESCE(host_id, zero-uuid)) WHERE state NOT IN ('succeeded','failed','cancelled') + UNIQUE INDEX platform_apply_attempts_request_uk (updater_request_id) WHERE updater_request_id IS NOT NULL + INDEXes platform_apply_attempts_history_idx / platform_apply_attempts_host_idx. THE TWO UNIQUE PARTIAL INDEXES ARE THE SINGLE-FLIGHT GUARANTEES and they live in the database rather than in code, exactly as job_runs_open_per_target does: platform_apply_runs_active_uk IS the run_active refusal (two admins pressing Apply at once produce a unique violation, not two fleets moving), platform_apply_attempts_open_target_uk IS the attempt_in_flight refusal and the wire's "single-flight per host: refuse, never queue". The zero UUID stands in for the control-plane target because a plain partial index on a nullable column would NOT make the NULL rows collide. host_id CASCADES (unlike every audit-shaped column here, which is SET NULL) so that the target/host_id CHECK stays a real invariant instead of one a host deletion silently breaks — DELETE /v1/hosts/{id} already cascades a forgotten host's GPUs and terminal session history, and this follows it. THE TWO release_id FKs POINT OPPOSITE WAYS ON PURPOSE: a run's is RESTRICT (a fleet-wide change must not be erasable by a release-cache cleanup) while an attempt's is SET NULL (a revert's digest set legitimately corresponds to no known release, and requested_digests, not the FK, is the authority for what an attempt did). previous_digests IS THE WHOLE REVERT MECHANISM — prototype #113 finding 5: the previous digests do restore a half-failed stack, but something has to drive the restore, so they must already be recorded before they are needed, which is why every release_state carries them and not only the failing ones. NUMBERING: 0075 is PROVISIONAL — this contract is authored ahead of the SQL, which lands with the apply tickets (#116/#117/#118); 0074 is taken by amendment 1 (#107/#110, landing concurrently), so 0075 is the next free number at authoring time. If it has been taken by then, the SQL lands at the next free number and this header records the correction rather than reusing a version, exactly as 0045, 0064 and 0074 did — a number already applied to a live database must never be reused, since golang-migrate keys on the version alone. ROLLBACK, standing repo rule: once applied, never deploy a control-plane binary embedding only <= 0074. The down migration drops both tables (attempts first, then runs); the loss is the entire apply history INCLUDING every recorded previous_digests set, so a revert becomes impossible for every host whose last apply predates the rollback — export platform_apply_attempts before rolling back any deployment that has applied a release.
  0076_platform_apply_cordons.up.sql -- (platform-release apply, #117, PURELY ADDITIVE — one column with a default on platform_apply_runs and nothing else; no wire change, no backfill.) ALTER TABLE platform_apply_runs ADD COLUMN cordoned_hosts JSONB NOT NULL DEFAULT '[]'::jsonb. A fleet run cordons every host for its control-plane step, and that step RECREATES THE CONTROL PLANE — so the pre-run scheduling state must be in the database, not in the process that is about to be replaced. Observed live: a failed run left every host `draining` because the record died with the old process. The down migration drops the column; a run in flight across that rollback loses only its record of which cordons to lift, which then need lifting by hand.
  0078_gpu_driver_identity.up.sql -- (encoder-certification driver identity, #144, PURELY ADDITIVE -- two nullable columns, no default, no backfill, no key change.) ALTER TABLE gpus ADD COLUMN driver_identity TEXT + ALTER TABLE host_encoder_certification ADD COLUMN driver_identity TEXT. The first is what the agent reports for that GPU right now (agent-api.md `capacity.gpus[].driver_identity`); the second is what it was when the bench ran, written from the gpus row by the upsert itself. NULL MEANS UNKNOWN ON BOTH, never "no driver": every row that exists when this runs is NULL and STAYS ELIGIBLE, so the migration changes no launch decision on its own -- caps start narrowing only as certifications are re-measured under an identity. Failing closed instead (dropping every legacy cap) would start sessions at rungs their host may not sustain, which is the worse failure. The down migration drops both columns; the loss is the ability to tell measurements from different drivers apart, i.e. the behaviour before this migration.
  0079_release_channel_beta.up.sql -- (beta release channel, amendment 3, #121, PURELY ADDITIVE -- ONE WIDENED CHECK and nothing else; no column, no table, no data change, no backfill.) ALTER TABLE instance_settings DROP CONSTRAINT instance_settings_release_channel_check, then re-ADD it as CHECK (release_channel IN ('stable','edge','beta')). NOTHING ELSE MOVES: platform_releases.channel keeps CHECK IN ('stable','edge'), because `beta` selects the rows the stable channel already caches and keeps the prereleases among them -- so a channel switch re-detects nothing and writes nothing, and no row's meaning changes. The listing and ordering rules `beta` adds are code, not schema (control-api.md §"Platform-release beta channel"): SemVer 2.0.0 precedence is inserted between schema_version and built_at in the ordering, and a release whose version orders below an installed PRERELEASE at the same schema_version is not offered -- which is what makes a switch back to `stable` wait for stable to catch up instead of proposing a downgrade (ADR 0002). The down migration folds any row on `'beta'` back to `'stable'` (the column default, and the channel beta's rows come from) and then narrows the CHECK again; the only loss is the operator's channel selection.
  0080_platform_release_webhook.up.sql -- (release notifications, amendment 4, #123, PURELY ADDITIVE -- two defaulted columns on instance_settings and one new table; no existing column changes, no backfill, and a behavioural no-op on upgrade because release_webhook_enabled defaults false. See control-api.md §"Release notifications" and §`platform_release_notifications` above.) ALTER TABLE instance_settings ADD release_webhook_enabled BOOLEAN NOT NULL DEFAULT false + release_webhook_url TEXT NOT NULL DEFAULT ''. CREATE platform_release_notifications (release_id UUID PK -> platform_releases ON DELETE CASCADE, status CHECK IN ('delivered','failed'), attempts INTEGER NOT NULL DEFAULT 0 CHECK >= 0, last_attempt_at TIMESTAMPTZ NOT NULL DEFAULT now(), last_status_code INTEGER NULL, last_error TEXT NULL, delivered_at TIMESTAMPTZ NULL) + CHECK ((status = 'delivered') = (delivered_at IS NOT NULL)). THE PRIMARY KEY IS THE DEDUPE GUARANTEE and it lives in the database, not in code: the claim is one INSERT ... ON CONFLICT DO UPDATE ... WHERE status = 'failed' AND attempts < cap, so a scheduled detection pass and a "Check now" racing on the same release cannot both send it. NO CHECK ON THE URL: a CHECK cannot express "https, no userinfo, a host we are willing to dial", and half of that rule in SQL would read as the whole rule -- the PATCH handler validates it and the delivery path re-checks it. THE SIGNING SECRET IS NOT IN THIS MIGRATION: it is an instance_secrets row (migration 0040), encrypted under a key that is never in the database, because a plaintext credential column would put the one thing a database dump must not yield straight into the dump. The down migration drops the table and both columns; the loss is the dedupe record, so a re-applied 0080 announces the currently-available release once more -- noisy, never wrong.
  0081_platform_auto_apply.up.sql -- (unattended automatic apply, amendment 8, #122, PURELY ADDITIVE -- two defaulted booleans, no new table, no existing column changed, and a behavioural no-op on upgrade because both default false. See control-api.md §"Platform-release apply".) ALTER TABLE instance_settings ADD platform_auto_apply BOOLEAN NOT NULL DEFAULT false. ALTER TABLE platform_apply_runs ADD unattended BOOLEAN NOT NULL DEFAULT false + partial INDEX platform_apply_runs_unattended_idx (release_id, state) WHERE unattended. THE `unattended` COLUMN IS THE FAILURE POLICY: without it, "has an unattended run already failed on this release" is unanswerable, because requested_by is NULL both for an unattended run and for one whose admin was later deleted. NO WINDOW COLUMN, DELIBERATELY -- the pass runs from the platform.release_detect job, so that job's editable schedule is the window; a second time model would need its own validation and rendering to say the same thing twice. The down migration drops both columns and the index; the loss is which runs were automatic, so the per-release suppression forgets itself and a previously-failed release may be attempted once more -- noisy, never corrupting. Export platform_apply_runs first if that history matters.
  0083_self_update_hardening.up.sql -- (self-update hardening, amendment 9, #185, PURELY ADDITIVE -- two widened CHECKs and two defaulted/nullable columns on existing tables; no new table, no backfill, no behaviour change for a run or attempt that never reaches the new values. See control-api.md §"Self-update hardening".) ALTER TABLE platform_apply_runs: state CHECK widened to add 'succeeded_partial'; ADD retry_of UUID NULL REFERENCES platform_apply_runs(id) ON DELETE SET NULL; ADD skipped JSONB NOT NULL DEFAULT '[]' CHECK (array, <= 16384 bytes). ALTER TABLE platform_apply_attempts: kind CHECK widened to add 'auto_revert'. THE `skipped` COLUMN IS WHAT MAKES `succeeded_partial` DECIDABLE AFTER A CRASH: the list was in-memory before and a partial run whose explanation evaporated would be a state with no reason; `retry_of` is provenance only and is read by nothing that decides a target. The down migration must first rewrite every row in the new vocabulary to its nearest old value (succeeded_partial -> succeeded, auto_revert -> revert) BEFORE narrowing the CHECKs, or the ALTER fails on a live table; then it drops the two columns. The loss is which runs were partial and which reverts were automatic -- history becomes vaguer, never wrong about what is installed. NUMBERING: 0083 is the next free number on develop at authoring time; PR #182 (#176) also claims it, and whichever lands second renumbers to 0084 -- a number already applied to a live database must never be reused, and golang-migrate only moves forward, so a stack that applied 0084 first would never apply an 0083 landing later. ROLLBACK, standing repo rule: once applied, never deploy a control-plane binary embedding only <= 0082.
...
`dockerfile` are strings (empty where inapplicable), and `build_args` is an
object (`{}` when empty). Compare identities using JSONB equality. A first
verified ready report inserts the current pair. On a later different-version
verification, `previous_version` and `previous_identity` copy the row's prior
...
`managed_home_claims_pending_home_session_idx` on `pending_home_session_id`
where non-NULL supports late terminal reconciliation after a session row is
deleted. The session ID has no FK to `sessions`: synthetic reaping, session
deletion, restart and host deletion cannot erase uncertain mount evidence.
...
trace, log or diagnostic export. Existing session APIs keep their own IDs.
...
canonical target, including another derived tile of the same parent, while
retaining the original token and timestamp; rejection or pre-send failure of
that later swap does not clear the earlier hold. A different canonical target
may take a separate hold. Every new launch, swap or local launch of a home
held by another session is refused with privacy-safe `home_conflict`; a new
launch never reuses a prior terminal session's hold.
The original session's first accepted assigned-to-running transition may
still materialize its claim under the signed mount-binding checks while its
...
the signed claim reservation; it does not prove no physical home exists.

Running, swap-complete and rollback callbacks have no operation ID and never
clear a hold, even if they commit the new `sessions.app_id`. A terminal
`session_state` clears holds for that historical session only when its
*reporting connection* advertised `terminal_home_cleanup_v1`, the report is
authenticated, its host equals `claims.host_id` (non-NULL), and it also equals
`sessions.host_id` when the session row still exists. Such an agent reports
terminal only after all original and swapped source containers are removed
and retains home refs until cleanup completes. A current agent without that
capability, an old agent, a NULL-host claim, a report from another host, or a
terminal report with uncertain cleanup leaves the hold for audited repair
(#347). A late qualified terminal on an already-terminal or deleted session
changes **only** matching claim hold columns; it never rewrites the session,
emits a session event, changes a reservation or materializes a claim. The
session ID is never reused, so a pre-swap terminal means that agent could not
...
The signed admin host-delete path is an explicit tombstone exception: it may
tombstone the host's `user_homes` rows while holds exist, then its 0090 host
trigger changes affected claims to `conflict/claim_owner_missing`, sets owner
host NULL and **retains** each hold. A NULL-host janitor may delete its
orphaned bookkeeping row but never the claim or hold. User or canonical-parent
hard deletion instead refuses while any associated hold exists; ordinary
...
errors remain failures. The 0090 down migration drops these three triggers
and functions, then the partial index and claim table, after dropping the
sessions binding columns and the signed host-delete trigger. It never deletes
backing homes or user data.
...
second owner. Deleting a user or canonical app cascades an **unheld** claim;
the 0090 held-claim delete guards refuse the delete until qualified cleanup
proof or #347 repair. Neither path deletes an existing `user_homes` tombstone
or backing store. Swap and local
