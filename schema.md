...
| `kind` | `TEXT` NOT NULL | `CHECK (kind IN ('apply','revert','auto_revert'))` *(widened by migration 0083, amendment 9)*. **A revert is an apply with an older digest set** — same wire message, same states, same reasons — so it is a `kind` on this row rather than a second table or a second `agent-api.md` message. The column exists so history can say which button was pressed, and for nothing else. `auto_revert` is the one kind no button produced: the host's updater restored the previous digests itself after a failed health wait (`agent-api.md` `release_state.restored`), and the control plane inserted this row **already terminal** beside the failed apply, with the failed apply's `previous_digests` as its `requested_digests` and vice versa. It is never driven over the wire and never holds the open-target index. |
