...
| `cordon_restore_attempted_at` | `TIMESTAMPTZ` NULL | *(migration 0084, amendment 10, #176)* when the boot sweep last tried this run's cleanup. An **ordering key, not a record**: the sweep is bounded per start (newest 20, one pass per start), so a plain newest-first selection would re-pick the same handful of permanently-failing runs forever and an older requirement behind them would never be looked at again. Never-attempted rows are claimed first, then the least recently attempted. **Not served.** |
