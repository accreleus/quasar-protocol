...
- **Errors** use a uniform body and conventional status codes:
  ...
  *P2-01* — no online host/GPU can serve the request), `capacity_exhausted` (503, *P2-01* — a
  ...
  `host_not_ready` (503, *amendment 11, #260* — a host/GPU that could otherwise serve the request
  is online, and the **only** reason none qualified is a failing evidence-based readiness check;
  the remedy is the host's admin, not the caller),
  `internal` (500). **Retryable:** `no_host_available`, `capacity_exhausted`, `host_not_ready`, `rate_limited`
...
| `PUT` / `DELETE /v1/admin/hosts/{id}/readiness-overrides/{check_id}` | **admin** | *(amendment 11, #260)* launch on this host despite one named failing readiness check / withdraw that decision — idempotent; `409` when the check is not an overridable block right now |
...
> ~~**ADVISORY WORDING ONLY** — it never gates registration, admission, or scheduling, and no
> endpoint anywhere refuses a request because a check failed.~~ **Superseded by amendment 11
> (#260, "Evidence-gated readiness" below):** registration is still never gated, and a check
> without `blocks` is still guidance only, but a check that rests on evidence and fails now
> excludes the launches it names from admission. `readiness_reported_at` is stamped on **every
> real report**, not only on a change (the shipped behaviour since migration 0062; the earlier
> "last changed" wording was wrong), which is what makes it usable as the gate's freshness
> signal. Canonical schema: `openapi.yaml` `Host` / `ReadinessCheck`.
...
|---|---|---|---|
| `updater_socket` | both | the target's recovery actor answers on its socket (the control socket for the control plane, the agent socket for a host) | the socket and the error, and where to look (the recovery actor's container and its log); no Compose command. A control plane with no recovery actor fails it, naming that; a host with none reports it not applicable, and `updater_absent` makes it ineligible. |
...
## Evidence-gated readiness — what may block a launch, `host_not_ready`, readiness overrides (amendment 11, #260, requires sign-off)

> **Amendment 11 (#260, under #210/#211; decision record: `quasar` ADR 0005 "only evidence gates a
> launch").** This is **not purely additive**: it withdraws one sentence the contract used to
> state absolutely — that host readiness *never* gates admission or scheduling. Everything else
> is additive: **three optional fields and one status** on a readiness check (`agent-api.md`
> `readiness`), **two always-serialized fields on the host body** (`readiness_gate`,
> `readiness_overrides`), **one retryable launch refusal** (`503 host_not_ready`), and **two
> admin-gated routes** (the readiness override). Persisted by migration **0085** (`schema.md`).
> A host whose agent predates this amendment sends no `blocks`, so nothing about it can block and
> its behaviour is exactly what it was.

### The rule

Registration is never gated: a host with every check failing still registers, because that is
what lets the console say why. A launch is affected by readiness in exactly one way:

> **A readiness check blocks only when it carries `blocks` and its `status` is `fail`.**

`blocks` is put there by the agent only on a check that rests on **evidence** — a host probe that
...
|---|---|
| `host` | every launch on the host |
| `homes` | every launch on the host that would mount a managed home |
| `gpu` | every launch that would be placed on the host's GPU whose index is `blocks.gpu_index` |
...
### Host body additions — both always serialized

`GET /v1/hosts`, `GET /v1/hosts/{id}` and the host list gain:
...
"readiness_overrides": [
  { "check_id": "audio_probe", "created_by": "<uuid|null>", "created_by_username": "alice",
    "created_at": "2026-09-19T10:00:00Z", "inert": false }
]
```
...
### Readiness override — admin

An admin's recorded decision to launch on one host despite **one named failing check**. It never
hides the check, applies to that `check_id` on that host only, and ends when the check next passes.
...
After each cleanup-capable registration and periodically at a capped interval
while that connection epoch stays active, the control plane rescans held
claims on that owner host with a terminal or missing session row and sends one
`session_stop` per distinct historical session ID on that epoch. Thus a
running row that becomes synthetically terminal after the registration scan,
including on the first heartbeat, enters the next same-connection scan. It
retries while the hold remains regardless of ack and repeats on later capable
reconnects. The capable agent returns a qualified terminal even if
...
**RH05 #345 exact cleanup responses (proposed amendment).** GET returns `200 HostImageCleanupView` even when a host is offline or inventory is incomplete. Its top-level `inventory_status` is `current`, `unknown`, or `offline`, with nullable `observed_at` and a safe `remedy`; an empty `images` array under non-current status means no conclusion about cache contents. Historical or unverified rows are omitted from `images` under non-current status; top-level status/remedy communicates uncertainty, so an empty array is never evidence of absence. Candidates are ineligible with `offline` or `unknown_inventory` and a remedy. It lists only exact versions whose ref and daemon image ID were verified in the current authenticated `image_cleanup_v1` inventory. Retained history without that evidence blocks cleanup; it never invents a candidate. Blocker codes are `required`, `container_reference`, `pending_launch`, `pending_image_operation`, `pending_template_work`, `retained_previous_success`, `unknown_inventory`, `offline`, and `removing`. For a present exact version, `container_reference` applies when a managed `running` or `stopping` session references the exact image ref **or** current authenticated agent inventory reports `container_referenced: true`, including stopped containers outside managed sessions. An omitted field on a `present` entry gives that candidate `unknown_inventory` rather than eligibility, with the candidate remedy `Upgrade the node agent to report container references, then refresh image inventory`; a 409 in this case reuses the frozen unknown-inventory message and supplies that upgrade remedy. `false` describes only the last complete scan. A change of this point-in-time reference evidence does not advance the cleanup fence generation. POST uses the latest current-connection snapshot while holding the fence row lock; `true` returns the existing `container_reference` 409, and omission returns the existing `unknown_inventory` 409. POST still depends on the agent's locked all-container recheck; a container created after preview can produce a durable `failed/reference_in_use` attempt without deleting the image. `retained_previous_success` means the exact `(previous_version, previous_identity)` pair from `host_image_success_history` on that host, populated only after a different adopted version reached ready; the retained identity is frozen and is not inferred from the current catalog. An eligible row has no blockers. The generation is a canonical decimal string from 0094; requirement/reference writers advance it.

POST requires exact image/version, frozen adopted ref, daemon image ID, and preview generation. A changed exact identity rejects as a stale preview even when the version label is unchanged. It rechecks all blockers under the fence row lock, increments the fence generation, creates a durable attempt with that post-increment generation, and commits `removing` before network dispatch. A new attempt returns `202 HostImageCleanupAttempt`. Before checking stale generation or blockers, POST looks for an attempt matching host, image, version, frozen ref and daemon ID whose persisted `generation = expected_generation + 1`; an active `removing`/`unknown` match returns that same attempt with `202`, and the latest retained terminal `removed` match returns it with `200`. All other stale generations or blockers return `409`. Stale generation, blocker or competing attempt returns `409 conflict` with a current blocker and remedy. Offline/incomplete inventory returns `409` without dispatch. Unknown host or image with no catalog, durable fence/history, or authenticated managed-version inventory identity is `404 not_found`; a catalog-pruned image still appears in a current verified agent inventory and can be explicitly cleaned using its durable exact identity. Selected app exact-ref requirements, containers, pending work, and frozen previous-success history continue to protect it. Malformed input is `400 validation_failed`. Frozen safe 409 messages are `Image cleanup preview is stale; refresh and try again`, `Image is required by an app`, `Image is used by a container`, `Image has pending launch, image or template work`, `Image is retained for recovery`, `Image inventory is unknown; reconnect the host`, `Host is offline; retry after reconnect`, and `Image cleanup is already in progress`. The conflict body includes the current candidate when available.

`GET /v1/admin/hosts/{id}/images/cleanup/attempts/{attempt_id}` is an admin-only read of one durable attempt, using the existing `HostImageCleanupAttempt` response with exact image/version/ref/daemon ID, generation, persisted `removing|unknown|removed|failed` state, and nullable safe operator reason code. The attempt must belong to the requested host; an absent, pruned, or other-host attempt returns `404` without revealing another host's state. A malformed host or attempt UUID is `400 validation_failed`; standard `401`/`403` apply. This read does not contact the agent, infer absence from a preview, replay a command, or change the attempt. `removing` means pending, `unknown` means the outcome is uncertain and requires reconnect/journal reconciliation, `failed` exposes its safe reason for an actionable retry after refreshing the preview, and only `removed` proves completion. Polling can stop only on a terminal state, `removed` or `failed`; `unknown` is not terminal. Offline or incomplete inventory never rewrites a terminal attempt. A fresh UI visit without an attempt ID cannot treat a missing preview candidate as confirmed removal. Attempts in `removing` or `unknown` are never pruned, and neither is the latest terminal attempt for each host/image/version/ref/daemon-ID identity. The control plane may prune an older terminal attempt only after a newer terminal attempt for that same identity exists. After pruning, this read returns `404` and the UI must say the prior outcome is unavailable. `reason` is `null` or exactly one of `identity_mismatch`, `inventory_unknown`, `reference_in_use`, `operation_busy`, `unsupported`, `image_still_present`. The control plane persists only these values; it stores any other agent-supplied reason as `null` and never forwards it. `retired_attempt` is never a persisted reason. `image_still_present` covers terminal failure after removal was attempted or fresh complete inventory proves the exact ref remains. No reason exposes raw daemon text, a path, a credential, or a diagnostic blob.

Attempt/fence transitions are exhaustive. POST commits `removing` with its post-increment generation. A matching terminal `removed` plus current complete daemon evidence changes attempt to `removed` and fence to `idle`. A first definite `ack{ok:false}` backed by a durable terminal refusal other than `retired_attempt` (which promises no delete began for this attempt), a matching terminal `failed` carrying that same first-refusal code, or terminal `failed` with current complete inventory proving the ref still present changes attempt to `failed` and fence to `idle`; the host's previous ready row is demoted and any current requirement is re-ensured. A lost ack, disconnected agent, uncertain terminal or incomplete inventory changes/keeps attempt `unknown` and fence `removing`. `ack{ok:false,error:"retired_attempt"}` is a duplicate answer only: it never terminates an attempt or proves its physical outcome. The attempt stays `removing`/`unknown` until a matching terminal report or the explicit retired-ID plus fresh complete inventory reconciliation resolves it. The control plane may retry delivery only while that persisted attempt is unresolved and only with the **same** attempt ID, generation, image/version/ref and daemon ID to recover lost acceptance; it never starts a second physical delete for a terminal attempt. The agent deduplicates that resend against its durable journal or retired tombstone, so one attempt executes at most one physical deletion. On reconnect or repeated identical POST, the control plane requests the complete attempt journal and fresh daemon inventory. A journal entry still `removing` keeps the fence removing even if the ref is present; physical removal may be in progress. Exact ref absence after the agent reports terminal and rechecks containers resolves to `removed`. Exact presence resolves to `failed` only after a terminal failure or a complete journal proves there is no live operation for that attempt. A complete journal that omits the attempt is authoritative only from a new/current authenticated connection after the agent has durably retired that requested missing attempt ID under cross-epoch cleanup serialization and reported the resulting snapshot; socket closure alone is not proof that an old handler cannot still run. Omission proves no live attempt only when that exact ID was listed in the request, appears in `retired_attempt_ids` from the fresh complete cross-epoch snapshot, and the agent acknowledged durable retirement; a generic empty journal snapshot is not enough. With a fresh complete `image_versions_state` snapshot from the same current authenticated connection whose exact target entry is `absent` (attesting a complete daemon ref and all-container scan with no container using the target runtime image ID), no live attempt and a retired journal ID, the exact ref absence resolves to `removed`/fence `idle` with an `image.cleanup.reconciled` audit event; any current requirement is re-ensured. Exact ref still present resolves to `failed`/fence `idle` and re-ensures any requirement. This also covers a command lost before journal write or lost agent state after deletion. Missing/stale journal, incomplete inventory, identity ambiguity or inability to prove old delivery closed remains `unknown`/fence `removing`; the operator remedy is to restore the capable agent's durable journal or repair its managed-image classification, reconnect, and rerun verification. The status never asserts success before those proofs. After persisting a matching terminal result, the control plane sends `image_cleanup_state_ack` to stop terminal re-reporting. Only a proven terminal state releases the fence. There is no `pending` or `cancelled` state.

The control plane sends only `image_cleanup` to a capable current connection. It confirms `removed` only from a matching terminal attempt/generation and current daemon evidence; ack and stale reports cannot confirm success. Lost ack or process restart recovers the persisted attempt and waits for agent journal reconciliation; any acceptance retry uses only the identical attempt ID, generation and identity and never creates another physical deletion. A new requirement during removal stays pending; ready/launch are blocked until re-ensure verifies the new identity. Legacy uninstall may remove only a proven unreferenced current version through legacy `image_remove`. With an old agent, retained previous success, unknown inventory or any blocker, it skips physical removal and reports retained cache. It may still drop adoption; it never deletes homes.
